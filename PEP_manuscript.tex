%% TO DO %%
% Make points about relative vs absolute motion as pertains to stability of a single Euler in the new section Challenges and opportunities in the application of paleomagnetic Euler pole analysis
% Update interpretation text related to comparison to TC2017 Say that introducing the TPW rotation enables the implied pole position from the model to match the longitude position. However, introducing that rotation slows the latitudinal different rate such that it is not as good of a match with the data and inversion.
% Describe that the TPW
...
%In most treatments, this number of stage poles is subject to researcher choice (however, see theoretical progress in this regard by \citet{Gallo2021a}).
...
A forward model describes how we generate predicted observations given a set of model parameters (Equation~\eqref{eq:forward}). The forward model for paleomagnetic Euler pole analysis in this study is essentially unchanged from that of \cite{Gordon1984a}. There are in general three scenarios of modeling plate motions (and hence paleomagnetic pole motions). A first scenario considers that the progression of a series of paleomagnetic poles to be the sole result of true polar wander along a great-circle path. In such a model, three parameters exist: a latitude and a longitude of the true polar wander rotation axis and a rotation rate. A second scenario concerns that an APWP is a result of multiple stages of small-circle rotations about a series of Euler poles. Each Euler pole has three parameters: a latitude, a longitude, and a rotation rate. In a model with multiple Euler poles, we also must specify the ages where one Euler pole switches to the next (the cusps, or ``hairpins'' of \cite{Irving1972a}) as a random variable. In the context of parameter inversion, these ages are often known as ``changepoints.'' Although these changepoints need to be defined to happen abruptly in time, the change in the trajectory of the apparent polar wander path can be either sharp or gradual. In a third scenario, a combination of multiple Euler pole rotations and true polar wander may be used to model paleomagnetic pole paths. 

In addition to rotational axes and rates, we also need to initiate a model with a starting position on the globe, which, in practice, can be sampled from the Fisher distribution of the oldest paleomagnetic pole in the data set. The starting point contributes two parameters (a latitude and a longitude). 

Therefore, a model only inverting for true polar wander rotation will have 3 parameters associated with the Euler pole position (longitude and latitude of the rotation axis and rate of the rotation). A model with $n_e$ Euler rotations will have $3 n_e$ parameters for the poles, $(n_e-1)$ parameters for the changepoints, and 2 parameters for the starting location. The number parameters for which we are inverting is then given by
\begin{equation}
\begin{aligned}
N &= 3 n_e + (n_e -1) + 2 \\
 &= 4 n_e + 1.
...
Two more parameters will be included in a model where true polar wander is considered to happen on top of $n_e$ Euler rotations. The reason for there being only two additional parameters is because in such cases, the axis of true polar wander is not independently defined, but is constrained to be on a great circle that is orthogonal to the vector representing the starting pole position. Therefore, a total of $4 n_e + 3$ parameters are inverted for. 
...
On top of these parameters associated with spherical rotations, we can also incorporate ages associated with each observed paleomagnetic pole as random variables in the forward model. In a case where a portions of the paleomagnetic poles have good age constraints (such as a radiometric date derived from the parent rock) whereas the rest of the poles have poorly constrained ages, the posterior distributions given by the Bayesian inversion model could provide an estimate on the ages of all poles in a particular model setup.
...
\textbf{Euler rotation axes:} 
The first parameter we consider is the position of the Euler poles, which should be drawn from a spherical probability distribution. The least informative prior distribution for an Euler pole in a forward model is the uniform spherical distribution:
\begin{equation}
...
To demonstrate the resolving power for recovering the Euler rotation axes and rotation rate of the inversion method, we introduce minimal prior knowledge on these parameters but assume the age of the paleomagnetic poles are well constrained. Therefore, we use a Watson girdle distribution with $\kappa_W$=-0.1 as the prior distribution for the Euler rotation axis, and a uniform distribution of $U(0, 4)$ as the prior distribution for the Euler rotation rate. 
...
The paleomagnetic data necessitate at least one change in the Euler pole between 60 Ma and the present. This need for more than one Euler pole is most readily visualized in the latitude of the poles which implies a change in rate between 60 Ma and the present (Fig. \ref{fig:Aus_Cenozoic_track}B). We apply the Bayesian inversion framework to invert for two Euler poles for Cenozoic Australia using prior probability constraints from these paleomagnetic poles. We use an uninformed prior probability for the timing of the changepoint between the two Euler poles in which it is assigned a uniform probability distribution between the age of the oldest ($\sim60$ Ma) and the youngest paleomagnetic poles ($\sim0.005$ Ma) in the compilation. We also use a weakly informative uniform prior distribution of $U(0,4)$ for the Euler rotation rate, as we know based on the position and age of the paleomagnetic poles that the overall plate motion rate for Australia in the Cenozoic is most likely to be less than 4\textdegree/Myr (Fig. \ref{fig:Aus_Cenozoic_track}A). For the prior distribution for the Euler rotation axes we use a Watson girdle distribution with $\kappa_W$=-0.1, representing nearly spherical uniform distribution. 

The posterior distribution of the Euler pole positions, samples of the small circle paths generated from the posterior distributions, the implied pole latitudes, and the full plate motion angular rotation rates are shown in Figure \ref{fig:Aus_Cenozoic_track}. The inverted pole latitude development history based on the paleomagnetic poles is more consistent with the global plate reconstruction model of \cite{Muller2016a} which considers hot spots to be stable with respect to the spin axis than the model of \cite{Torsvik2017a} which interprets there being relative motion between hot spots and the spin axis (Fig. \ref{fig:Aus_Cenozoic_track}B). Given that a portion of the apparent plate motion must be partitioned into interpreted hot spot motion in the model of \cite{Torsvik2017a}, it is expected that the absolute motion of a plate in that model would be slower than the apparent speed. However, the \cite{Torsvik2017a} model still fails to fully reconstruct paleomagnetic pole latitudes, particularly at older ages (Fig. \ref{fig:Aus_Cenozoic_track}B). 

In contrast, Fig. \ref{fig:Aus_Cenozoic_track}C shows that the inverted paleomagnetic pole paths based on the Bayesian model match better with the reconstruction of \cite{Torsvik2017a} than that of \cite{Muller2016a}. At younger ages ($<$38 Ma), both global plate models give similar reconstructions for the paleomagnetic pole paths that are in good positional correspondence with those predicted by the Bayesian posterior results (Fig. \ref{fig:Aus_Cenozoic_track}C). This is illustrated by their overlapping Euler pole positions in Fig. \ref{fig:Aus_Cenozoic_track}C. During older times (ca. 38-60 Ma), the shorter arc length and the sparse paleomagnetic pole constraints result in the position of the older Euler pole being more poorly resolved than the younger, resulting in the more sporadic spherical distribution density for the older Euler pole position shown in Fig. \ref{fig:Aus_Cenozoic_track}C. Despite this, the older portion of the inverted paleomagnetic pole paths match better with the model of \cite{Torsvik2017a} as they do not show sharp longitudinal bends toward the east (Fig. \ref{fig:Aus_Cenozoic_track}C). 

Although discrepancies between the global plate motion models are expected given that \cite{Torsvik2017a} take into account motion of hot spots relative to the spin axis using the model of the Global Moving Hotspot Reference Frame (GMHRF; \citep{Doubrovine2012a}) and \cite{Muller2016a} do not, it is surprising that the inversion results based solely on paleomagnetic poles fit with neither global plate model in both pole longitude and pole latitude. One cause for such discrepancies could be the uncertainties associated with the calculation of the speed and direction of hot spot motion in the GMHRF. As \cite{Wang2019a} pointed out, the GMHRF model actually provides worse fits for many hot spot tracks around the globe, including the East Australia hot spot and Lord Howe Hotpot, than a fixed hot spot reference frame does. While it is likely that are relative motions between hot spots and deep mantle, the estimates for such motions by \cite{Doubrovine2012a} could be erroneous such that the reconstructed Australia plate motion in the Cenozoic still has a mismatch with the paleomagnetic data (Fig. \ref{fig:Aus_Cenozoic_track}). In addition, the GMHRF is determined by starting at present and calculating backward in time, it would tend to be more accurate and reliable for more recent geologic past. That the model of \cite{Torsvik2017a} tends to have more discrepancy in older pole latitudes with those of \cite{Muller2016a} model and the observed paleomagnetic poles could be reflecting such decrease in accuracy of the GMHRF calculations through older times.

Despite the discrepant paleomagnetic pole path reconstructions of the global plate models, they nevertheless both suggest a acceleration for Australia's plate motion in the later portion of the Eocene Epoch (Fig. \ref{fig:Aus_Cenozoic_track}). Our inversion result is consistent with those reconstructions as it also recovers an acceleration in Euler pole rotation rate (Fig. \ref{fig:Aus_Cenozoic_track}B ,D). The posterior distributions for Euler rotation rates have median values that increase from 0.3$^\circ$/Myr to 0.8$^\circ$/Myr in agreement with the \cite{Muller2016a} model (Fig. \ref{fig:Aus_Cenozoic_track}D). The 95\% credible interval for the age of the changepoint associated with this change in rate spans from ca. 51 to 30 Ma with a median 40.8 Ma. While this broad posterior distribution reflects the sparse constraints between 60 and 40 Ma, the timing of the change in rate is consistent with the \cite{Muller2016a} and \cite{Torsvik2017a} models (Fig. \ref{fig:Aus_Cenozoic_track}B, D).

Overall, the analysis highlights a broad agreement in the interpretation of kinematics that results from inversion of the Cenozoic paleomagnetic poles for Australia and that which results from seafloor-based plate motion models.

% The inverted APWPs can be compared to the \cite{Muller2016a} model where the global moving hotspot reference frame of \cite{Torsvik2008a} is fixed relative to the spin axis and the \cite{Torsvik2017a} model where relative rotations are implemented (Fig. \ref{fig:Aus_Cenozoic_track}). 

% In contrast, the younger Euler pole position is more tightly constrained through the inversion with the posterior distribution of the younger Euler pole positions (red density in Fig. \ref{fig:Aus_Cenozoic_track}B) concentrated near the Euler pole positions extracted from the \cite{Muller2016a} model (red stars in Fig. \ref{fig:Aus_Cenozoic_track}C).


% There is good positional correspondence in the younger inverted Euler pole position from ca. 38 Ma to the present as can be seen through comparison of the red density of the inverted Euler pole positions with the 

% which is the time interval where there is a higher concentration of paleomagnetic poles (comparison of red Euler pole position density to red Euler pole stars in Fig. \ref{fig:Aus_Cenozoic_track}C). The inversion is also successful in recovering the change in rate seen in both models where Australia's plate motion accelerated in the later portion of the Eocene (Fig. \ref{fig:Aus_Cenozoic_track}B). The posterior distributions for Euler rotation rates have median values that increase from 0.3$^\circ$/Myr to 0.8$^\circ$/Myr in agreement with the \cite{Muller2016a} model (Fig. \ref{fig:Aus_Cenozoic_track}D). The 95\% credible interval for the age of the changepoint associated with this change in rate spans from ca. 51 to 30 Ma with a median 40.8 Ma. While this broad posterior distribution reflects the sparse constraints between 60 and 40 Ma, the timing of the change in rate is consistent with the \cite{Muller2016a} and \cite{Torsvik2017a} models (Fig. \ref{fig:Aus_Cenozoic_track}B).

% correspond well with the pole path from the \cite{Muller2016a} model during the time interval where there is a higher concentration of paleomagnetic poles (ca. 35 Ma to the present; Fig. \ref{fig:Aus_Cenozoic_track}B,C). While the coverage of paleomagnetic poles is more sparse in the preceding time, the inversion is successful in recovering rates of motion and changes in this motion that are in agreement with the seafloor-based plate motion model (Fig. \ref{fig:Aus_Cenozoic_track}B,D). In particular, the inversion supports the interpretation that Australia's plate motion accelerated in the later portion of the Eocene. 

% In terms of the Euler pole positions, the shorter track length and the sparse paleomagnetic pole constraints result in the position of the older Euler pole being poorly resolved. In addition, the available ca. 60 Ma paleomagnetic pole does not imply the more eastward pole positions in the pole path ca. 60 Ma and instead has a position that agrees better with the previous seafloor data based model of \cite{Seton2012a}. 
...
An impetus for the development of this Bayesian paleomagnetic Euler pole inversion method was to constrain the absolute rates of plate motion associated with the ca. 1109 to 1070 Ma Keweenawan Track of paleomagnetic poles \citep{Halls1981a, Swanson-Hysell2009a, Swanson-Hysell2019a}. These poles are associated with the rapid motion of Laurentia towards the equator leading up to the Grenvillian orogeny and the assembly of the supercontinent Rodinia \citep{Swanson-Hysell2021a}. While previous approaches had established estimates for the rate of latitudinal motion associated with the Keweenawan Track \citep{Davis1997a, Swanson-Hysell2014b}, this Bayesian inversion method was implemented in \cite{Swanson-Hysell2019a} in order to constrain absolute rates (without providing the level of detail on the methodology that is elucidated in the present contribution). In addition to our goals of constraining absolute rates, this approach was particularly appropriate for the Keweenawan Track as it includes poles with quite disparate precision on their age constraints as some are constrained tightly by high-precision U-Pb dates \citep[e.g.][]{Fairchild2017a} while others have looser stratigraphic constraints. A technical note related to the analysis in \cite{Swanson-Hysell2019a} is that it used a version of the code written in PyMC2 (\citealp{Patil2010a}; \url{https://github.com/ian-r-rose/mcplates}). The PyMC project has migrated to PyMC3 which is a total rewrite of the module \citep{Salvatier2016a} and necessitated our code to refactored into its present PyMC3 version (\url{https://github.com/Swanson-Hysell-Group/Bayesian_PEP_inversion}). These two versions of the code apply the same methodology and recover very similar solutions. One change that we adopted in this study is that we applied inclination shallowing corrections to the sedimentary paleomagnetic poles within the Keweenawan Track (i.e. the Upper Freda Formation pole and the Nonesuch Formation pole of \cite{Henry1977a}). The reason to apply this correction is that global paleogeography reconstruction studies have found that inclination shallowing associated with sedimentary records almost always exist and can obscure paleogeography reconstructions if they are left uncorrected \citep{Domeier2012}. The correction was not applied by \cite{Swanson-Hysell2019a} due to the lack of site-level paleomagnetic data associated with the legacy literature. Here we correct both sedimentary poles with an inclination shallowing factor of 0.6, based on the compilation of \cite{Bilardello2016a} for hematite-bearing red beds. This value is also advocated by \cite{Domeier2012a} for correcting legacy sedimentary paleomagnetic poles where site-level data are not available as such correction helps resolves many discrepancies between paleomagnetic and geologic records for the amalgamation of Pangea. 

\begin{figure*}
\centering
\includegraphics[width=0.7\textwidth]{fig_Keweenawan_1.pdf}
\caption{A) Bayesian paleomagnetic Euler pole inversion of the Keweenawan Track based on forward models that incorporate one Euler pole or one true polar wander pole. The Euler pole locations and representative predicted pole positions associated with posterior Euler parameters and pole ages are shown with the observed paleomagnetic poles. The distribution of plate speeds associated with each model are shown in the histograms. B) Posterior likelihood distributions for all forward models used to invert for the Keweenawan Track. The log likelihood values are calculated by taking the natural log of equation \ref{eq:model_likelihood}.}
...
\caption{A) Bayesian paleomagnetic Euler pole inversion of the Keweenawan Track based on forward models that incorporate two Euler poles or one Euler pole with true polar wander. The Euler pole locations, representative predicted pole positions, and the distribution of plate speeds associated with each model are shown in the same way as in Fig. \ref{fig:Keweenawan_Track_1}. B) Top: prior pole age distributions used in the forward model. Bottom: posterior pole age distributions resulting from the MCMC inversion.}
...
Paleomagnetic Euler pole inversions to the Keweenawan Track are shown in Fig. \ref{fig:Keweenawan_Track_1}, \ref{fig:Keweenawan_Track_2}. The posterior distribution of the Euler pole positions are shown along with a sampling of the paleomagnetic poles generated from the posterior Euler parameters and pole ages which are plotted over the paleomagnetic poles. The prior distributions for the ages of the poles are shown in Fig. \ref{fig:Keweenawan_Track_2}B. The prior probabilities assigned to the ages illustrate the variable age uncertainties associated with the paleomagnetic poles in the Keweenawan Track. The posterior distribution of the plate speed for Laurentia resulting from the inversions are shown as angular velocities along with their 95\% credible intervals (Fig. \ref{fig:Keweenawan_Track_1}, \ref{fig:Keweenawan_Track_2}). To compare different forward models and assess whether the results of the inversions represent good fits for the observed paleomagnetic poles, we calculate the distribution of the natural log of the likelihood for the paleomagnetic poles generated from the posterior Euler parameters and pole ages based on the observed paleomagnetic poles. A summary of the posterior likelihood distributions is shown in Fig. \ref{fig:Keweenawan_Track_1}B. 

Applying a single paleomagnetic Euler pole inversion to the entirety of the Keweenawan Track results in a median rate of 3.1 \textdegree /Myr with a 95$\%$ credible interval of 2.7-3.6 \textdegree /Myr (Fig. \ref{fig:Keweenawan_Track_1}). Also shown in Figure \ref{fig:Keweenawan_Track_1} is an inversion with only true polar wander that results in a slower rotation rate. However, the samples of paleomagnetic poles generated from the posterior true polar wander rotation parameters deviate from the observed paleomagnetic poles, as is illustrated by the resampled poles for younger ages falling outside of the A$_95$ uncertainty ellipses of the observed paleomagnetic polesin Fig. \ref{fig:Keweenawan_Track_1}A. As a result, the posterior log likelihood values for the true polar wander model are much smaller than those for the one Euler pole model Fig. \ref{fig:Keweenawan_Track_1}B, representing that the resampled paleomagnetic poles from the true polar wander model fit the observed poles worse than those from the one Euler pole model. Shown in Figure \ref{fig:Keweenawan_Track_2}A are two other forward models considering an inversion with two Euler poles and an inversion with one Euler pole and true polar wander. The two Euler pole inversion results in a ca. 1097 Ma changepoint with rates that slow from $\sim$3.1\textdegree /Myr to $\sim$1.8\textdegree /Myr after the changepoint---a change that could be associated with the onset of collisional Grenvillian orogenesis. On the other hand, the forward model involving one Euler pole rotation and true polar wander partitions the relatively fast Euler rotation rate estimated by the one Euler pole model into a slower Euler rotation and a slow great circle rotation (Fig. \ref{fig:Keweenawan_Track_2}A). In addition, because the true polar wander involves the rotation of the solid Earth with respect to the spin axis, its allows more uncertainty associated with the location of the Euler pole, making the posterior Euler pole distribution more scattered. (Fig. \ref{fig:Keweenawan_Track_2}A).

Overall, forward models involving small circle Euler rotations always yield better fits to the observed paleomagnetic data than the model that considers only true polar wander great circle rotation. In comparison with models that involve one Euler pole rotation, the two Euler pole inversion results in a significant improvement in fit as can be visualized by the modeled pole positions shown in Figure \ref{fig:Keweenawan_Track}A and by the calculated posterior likelihood distributions for the paleomagnetic pole positions in Figure \ref{fig:Keweenawan_Track_1}B. Adding an additional true polar wander component to the two Euler pole inversion does not significantly improve fits to the observed paleomagnetic poles, but would rather result in more poorly constrained Euler pole positions due to the introduction of a solid Earth rotation. Therefore, we briefly summarize results from that model in Figure \ref{fig:Keweenawan_Track_1}B. 

% Inversion results from these models indicate that small circle Euler rotation is most likely required for the Keweenawan Track.
% , and estimated rotation rates are significantly faster than the fastest Cenozoic plate speeds and above the 15 cm/yr threshold that \cite{Zahirovic2015a} concludes requires exceptional explanation for plates with substantial continental lithosphere.
...
%Describe how relative plate motions are stable in Muller, but absolute is adding multiple Euler can decrease stability. Also can mention how some analyses of marine magnetic anomaly-data DeMets and Merkouriev (GJI, 2021) imply more rapid changes
% discussion how determining changepoint and the number of changepoints is a major challenge --- particularly since there could be "change intervals" rather than "changepoints" as noted. 
