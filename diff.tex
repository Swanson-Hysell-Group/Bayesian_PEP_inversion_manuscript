...
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF < % AGUJournalTemplate.tex: this template file is for articles formatted with LaTeX
%DIF < %
%DIF < % This file includes commands and instructions
%DIF < % given in the order necessary to produce a final output that will
%DIF < % satisfy AGU requirements, including customized APA reference formatting.
%DIF < %
%DIF < % You may copy this file and give it your
%DIF < % article name, and enter your text.
%DIF < %
%DIF < %
%DIF < % Step 1: Set the \documentclass
%DIF < %
%DIF < %
%DIF < 
%DIF < %% To submit your paper:
%DIF -------
...
%DIF 24-35d8
%DIF < %%%%%%%
%DIF < % As of 2018 we recommend use of the TrackChanges package to mark revisions.
...
%DIF 38-63d10
%DIF < 
%DIF < %% Enter journal name below.
%DIF < %% Choose from this list of Journals:
%DIF < %
%DIF < % JGR: Atmospheres
%DIF < % JGR: Biogeosciences
%DIF < % JGR: Earth Surface
%DIF < % JGR: Oceans
%DIF < % JGR: Planets
%DIF < % JGR: Solid Earth
%DIF < % JGR: Space Physics
%DIF < % Global Biogeochemical Cycles
%DIF < % Geophysical Research Letters
%DIF < % Paleoceanography and Paleoclimatology
%DIF < % Radio Science
%DIF < % Reviews of Geophysics
%DIF < % Tectonics
%DIF < % Space Weather
%DIF < % Water Resources Research
%DIF < % Geochemistry, Geophysics, Geosystems
%DIF < % Journal of Advances in Modeling Earth Systems (JAMES)
%DIF < % Earth's Future
%DIF < % Earth and Space Science
%DIF < % Geohealth
%DIF < %
%DIF < % ie, \journalname{Water Resources Research}
%DIF -------
...
%DIFDELCMD < %%%
\DIFdelend \title{Bayesian paleomagnetic Euler pole inversion for paleogeographic reconstruction and analysis}
...
%DIFDELCMD < %%%
\DIFdelend \authors{Ian R. Rose\affil{1}, Yiming Zhang\affil{1}, and Nicholas L. Swanson-Hysell\affil{1}}
...
%DIF < \affiliation{=number=}{=Affiliation Address=}
...
%DIFDELCMD < %%%
\DIFdelend \correspondingauthor{Nicholas L. Swanson-Hysell}{swanson-hysell@berkeley.edu}
...
%DIF <   Each must be 140 characters or fewer with no special characters or punctuation and must be complete sentences
%DIFDELCMD < 
...
%DIF <  \item	Each must be 140 characters or fewer with no special characters or punctuation and must be complete sentences
%DIF <  \end{keypoints}
%DIFDELCMD < 
...
%DIF < % ------------------------------------------------------------------------ %%
%DIF < 
%DIF <   ABSTRACT and PLAIN LANGUAGE SUMMARY
%DIF < 
...
%DIFDELCMD < %%%
%DIF <  The Plain Language Summary should be written for a broad audience,
%DIF <  including journalists and the science-interested public, that will not have 
%DIF <  a background in your field.
%DIF < 
%DIF <  A Plain Language Summary is required in GRL, JGR: Planets, JGR: Biogeosciences,
%DIF <  JGR: Oceans, G-Cubed, Reviews of Geophysics, and JAMES.
%DIF <  see http://sharingscience.agu.org/creating-plain-language-summary/)
%DIF < 
%DIF < % ------------------------------------------------------------------------ %%
%DIFDELCMD < 
...
Apparent polar wander paths (APWPs) synthesized from paleomagnetic poles provide the most direct data for reconstructing past paleogeography and plate motions for times earlier than ca. 200 Ma. In this contribution, we describe a new method for APWP synthesis that extends the paleomagnetic Euler pole analysis of \citeA{Gordon1984a} by placing it within the framework of a Bayesian inverse problem. This approach \DIFdelbegin \DIFdel{allows uncertainties in both pole position and age to be incorporated into the synthesis---uncertainties }\DIFdelend \DIFaddbegin \DIFadd{incorporates uncertainties in pole positions and age }\DIFaddend that are often ignored in standard treatments. The paleomagnetic Euler poles resulting from the inversions provide estimates for full-vector plate motion (\DIFdelbegin \DIFdel{in }\DIFdelend both latitude and longitude) and associated uncertainty. The method allows for inverting for one or more Euler poles with the timing of changepoints \DIFdelbegin \DIFdel{from one Euler pole to another }\DIFdelend being solved as part of the inversion. In addition, the method allows the incorporation of true polar wander rotations\DIFdelbegin \DIFdel{on top of Euler rotations}\DIFdelend , thus providing an avenue for probabilistic partitioning of \DIFdelbegin \DIFdel{absolute plate motions }\DIFdelend \DIFaddbegin \DIFadd{plate tectonic motion }\DIFaddend and true polar wander based on paleomagnetic poles. We show \DIFdelbegin \DIFdel{several }\DIFdelend example inversions on \DIFdelbegin \DIFdel{simple }\DIFdelend synthetic data to demonstrate the \DIFdelbegin \DIFdel{capabilities of the method}\DIFdelend \DIFaddbegin \DIFadd{method's capabilities}\DIFaddend . We illustrate application of the method to \DIFdelbegin \DIFdel{the Cenozoic database of }\DIFdelend \DIFaddbegin \DIFadd{Cenozoic }\DIFaddend Australia paleomagnetic poles which can be compared to independent plate reconstructions\DIFdelbegin \DIFdel{developed using seafloor data}\DIFdelend . A two-Euler pole inversion for the \DIFdelbegin \DIFdel{Cenozoic Australia record recovers the }\DIFdelend \DIFaddbegin \DIFadd{Australian record recovers }\DIFaddend northward acceleration of Australia in the Eocene with rates that are consistent with plate reconstructions. We also apply the method to constrain rapid rates of motion for cratonic North America associated with the Keweenawan Track of late Mesoproterozoic paleomagnetic poles. The application of Markov chain Monte Carlo methods to estimate paleomagnetic Euler poles can open new directions in quantitative paleogeography.
\end{abstract}

\section*{Plain Language Summary}
Movement of Earth's tectonic plates results in large changes in continent positions over time known as paleogeography. Reconstructing paleogeography is central for understanding long term changes to Earth's surface and interior. Ocean floor data are very useful for determining these motions, but only exist for the most recent 4$\%$ of Earth's history. For older times, we use magnetic directions in rocks that \DIFdelbegin \DIFdel{shows }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend positions relative to the north pole. These records are called paleomagnetic poles and plate motions result in progressions of paleomagnetic poles called paths. Earth scientists need ways to synthesize poles into paths. Ideally, such a synthesis uses uncertainties on pole positions and ages. We developed a new way to reconstruct plate motion from paleomagnetic poles using an approach called Bayesian inversion. We apply the method to pretend data to illustrate its capabilities. We then apply the method to real data such as that from Australia over the past 60 million years. This analysis shows change in the continent's position and the rate of its motion that is similar to approaches that rely on ocean floor data. For older times, the method can help reconstruct positions and provide estimates of plate motion rates and associated uncertainty. 
...
To demonstrate the ability to recover the Euler rotation axes and rotation rate using the inversion method, we introduce minimal prior knowledge on these parameters\DIFaddbegin \DIFadd{, }\DIFaddend but assume the ages of the paleomagnetic poles are well constrained. Therefore, we use a Watson girdle distribution with $\kappa_W$=-0.1 (more weakly informative than \DIFdelbegin \DIFdel{-0.8 }\DIFdelend \DIFaddbegin \DIFadd{-0.7 }\DIFaddend of Fig. \ref{fig:euler_pole_prior} and approaching a uniform distribution \DIFaddbegin \DIFadd{as illustrated in Fig. S2}\DIFaddend ) as the prior distribution for the Euler rotation axis, and a uniform distribution of $U(0, 4)$ as the prior distribution for the Euler rotation rate (in $^\circ$/Myr). 

The results of the inversion are shown in Figure \ref{fig:synthetic_pep}. The Bayesian approach successfully recovers a posterior probability distribution for the position of the Euler pole, as well as a rate that is centered near the true value of $1^\circ$/Myr (Fig.~\ref{fig:synthetic_pep}B). The posterior distribution for the rate has a highest posterior density credible interval at 95\% (which we abbreviate from here as a 95\% credible interval) between $0.7^\circ$/Myr and $1.2^\circ$/Myr, reflecting the resolving power of the inversion for data with the given uncertainties. An example of this same inversion with simulated noise on the age and position of paleomagnetic poles is \DIFdelbegin \DIFdel{shown }\DIFdelend \DIFaddbegin \DIFadd{developed }\DIFaddend in PEP$\_$synthetic.ipynb within the archived code repository and \DIFaddbegin \DIFadd{shown in Figure S1. It }\DIFaddend is also successful in recovering the true values within the credible intervals of the posterior distributions.

\begin{figure*}
\centering
\includegraphics[width=0.9\textwidth]{fig_synthetic_pep.png}
\caption{Inversion for Euler poles from synthetic data. (A) Five paleomagnetic poles are generated during a net $100^\circ$ rotation about an Euler pole at $00^\circ$N, $000^\circ$E over 100 Myr, for a rotation rate of $1^\circ$/Myr. The blue distribution is the probability density of Euler pole positions recovered by MCMC inversion, and the blue arcs are 100 of the resulting synthetic APWPs (sampled from 50,000). \DIFaddbeginFL \DIFaddFL{The color bar scale associated with the probability density is shown in a supporting information version of this figure }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{Fig. S3). Such supporting information figures are provided for every Euler pole position plot in the paper (Figs S3 to S6). (}\DIFaddendFL B) Posterior probability density for the rotation rate of the Euler pole recovered by the inversion. The solid line shows the median of the distribution ($0.96^\circ$/Myr), and the dashed lines show the 95\% credible interval ($0.70^\circ-1.23^\circ$/Myr). (C) Paleomagnetic poles generated using two distinct Euler poles. The first Euler pole is located at $10^\circ$S, $000^\circ$E (blue star), and rotates at $1.5^\circ$/Myr for 60 Myr. The second Euler pole is located at $10^\circ$N, $300^\circ$E (red star), and rotates at $0.75^\circ$/Myr for 60 Myr. The blue and red distributions show the posterior estimate location of the first and second Euler poles (respectively) recovered by the MCMC inversion. The blue and red arcs are 100 of the synthetic APWPs. (D) Posterior probability density for the rotation rates of the Euler poles recovered by the inversion. The solid lines show the median values of the distributions (\DIFdelbeginFL \DIFdelFL{$\sim 1.45^\circ$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\sim1.45^\circ$}\DIFaddendFL /Myr and 0.76$^\circ$/Myr for Euler rotation 1 and 2), and the dashed lines show the 95\% credible intervals.}
...
\DIFadd{The code implementing the analysis and developing the visualizations of this study are documented within Jupyter notebooks \mbox{%DIFAUXCMD
\cite{Kluyver2016a} }\hspace{0pt}%DIFAUXCMD
that are available on Github ( }\url{https://github.com/Swanson-Hysell-Group/Bayesian_PEP_inversion}\DIFadd{) and Zenodo (}\url{https://doi.org/10.5281/zenodo.7087844}\DIFadd{). These notebooks utilize the functions within the bayesian$\_$pep library within the repository and data that are within the repository as well. The computational environment used for running these notebooks and conducting the analysis can be reproduced using the environment.yml file within the repository. Instructions associated with recreating this computational environment can be found in the README.md file within the repository. MCMC analysis was performed using PyMC \mbox{%DIFAUXCMD
\cite{Salvatier2016a}}\hspace{0pt}%DIFAUXCMD
. Additional analysis was performed using the Python packages NumPy \mbox{%DIFAUXCMD
\cite{Harris2020a}}\hspace{0pt}%DIFAUXCMD
, Scipy \mbox{%DIFAUXCMD
\cite{Virtanen2020a}}\hspace{0pt}%DIFAUXCMD
, and PmagPy \mbox{%DIFAUXCMD
\cite{Tauxe2016a}}\hspace{0pt}%DIFAUXCMD
. Figures were created using Matplotlib \mbox{%DIFAUXCMD
\cite{Hunter2007a} }\hspace{0pt}%DIFAUXCMD
and Cartopy \mbox{%DIFAUXCMD
\cite{Met-Office2010a}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFaddend \acknowledgments
This work was supported by National Science Foundation grants EAR-1246670 and EAR-1847277. We thank Bruce Buffett, David Evans, and Mike Tetley for useful discussions during development of the method. We thank Mat Domeier, Facu Sapienza, \DIFaddbegin \DIFadd{Leandro Gallo, }\DIFaddend and other members of the CEED YC2.0 workshop for informative discussions during the \DIFdelbegin \DIFdel{final }\DIFdelend writing of the manuscript. \DIFaddbegin \DIFadd{These interactions were supported by a grant from the Peder Sather Center for Advanced Study. }\DIFaddend We also acknowledge Bram Vaes for an insightful review that improved the manuscript as well as the useful comments of an anonymous reviewer\DIFdelbegin \DIFdel{. MCMC analysis was performed using PyMC \mbox{%DIFAUXCMD
\cite{Salvatier2016a}}\hspace{0pt}%DIFAUXCMD
. Additional analysis was performed using the Python packages NumPy \mbox{%DIFAUXCMD
\cite{Harris2020a}}\hspace{0pt}%DIFAUXCMD
, Scipy \mbox{%DIFAUXCMD
...
and Cartopy \mbox{%DIFAUXCMD
\cite{Met-Office2010a}}\hspace{0pt}%DIFAUXCMD
. The code implementing the analysis and developing the visualizations are documented within Jupyter notebooks \mbox{%DIFAUXCMD
\cite{Kluyver2016a} }\hspace{0pt}%DIFAUXCMD
that are available on Github ( }%DIFDELCMD < \url{https://github.com/Swanson-Hysell-Group/Bayesian_PEP_inversion}%%%
\DIFdel{) and Zenodo (}\textit{\DIFdel{URL TO BE ADDED IN PROOFS}}%DIFAUXCMD
\DIFdel{)}\DIFdelend . 
\clearpage
